...
import { NativeStackProps, homeRefObject } from '../../../Common/commonType';
...
const HomeFavBulletin = forwardRef((props: any, ref: Ref<homeRefObject>) => {
  ...
  const loadPostArray = async () => {
    ...
    const PostArray: postArrayProperty[] = [];

    for (const bulletin of userBulletinList) {
  ...
  return (
    <StyledBorderContainer>
      <TopContainer text='즐겨찾는 게시판' onPress={goToBulletinList} isMore={true} />
      {renderPostArray()}
    </StyledBorderContainer>
  );
});
